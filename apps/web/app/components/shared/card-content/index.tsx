...
const buttonHoverEffect = `px-2 border-2 rounded-3xl border-transparent hover:border-mango hover:transition-colors hover:duration-1000`;

const CardContent: React.FC<CardDetail> = ({ isHero, slug, category, title, subtitle, authorName, date, tags }) => {
  return (
    <article className="flex flex-col justify-start gap-4 p-4 text-dune">
      <header className="flex flex-col justify-start gap-4">
        <Link href={category.url} className={`${fredoka.className} font-semibold text-xl`}>
          {category.name}
        </Link>
        <Link href={slug} className={`${robotoSerif.className} text-dune-emphasized font-bold ${isHero ? "sm:text-32px text-2xl" : "text-2xl"}`}>
          <h2>{title}</h2>
        </Link>
      </header>
      ...
      <footer className="grid sm:grid-cols-3 grid-cols-2 gap-2">
        ...
        <div className={`${cardMetaContent}`}>
          ...
          <time dateTime={date} className="px-2 border-2 rounded-3xl border-transparent">
            {date}
          </time>
        </div>
