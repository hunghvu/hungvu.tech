...
const getOpenwrtTohAvailableValuesOfEachField = async (): Promise<any> => {
  let res;
  try {
    res = await fetch(
      `${process.env.NEXT_REQUEST_CMS_OPENWRT_TOH!}/available-values-of-each-field`,
...
export default getOpenwrtTohAvailableValuesOfEachField;