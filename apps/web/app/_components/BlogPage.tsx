...
const BlogPage: React.FunctionComponent = () => {
  ...
  const customizedOpposite = (item: any) => {
    return (
      <aside className='flex lg:flex-col items-end gap-2 text-[#ffffffde]/60'>
        {/* TODO: Use proper time format from the API */}
        <div className='flex flex-row lg:flex-row-reverse gap-1 mr-8 lg:mr-0 items-center justify-center'>
          <span className='pi pi-calendar'></span>
          <time className='text-xs md:text-sm lg:text-base min-w-[85px]' dateTime={item.date}>{item.date}</time>
        </div>


        {/* TODO: Use db id as key instead */}
        {item.tags.map((tag: string) => <Tag key={tag} value={tag}
          severity='info' aria-label={`The article ${item.title} belongs to the topic ${tag}.`}
          pt={{
