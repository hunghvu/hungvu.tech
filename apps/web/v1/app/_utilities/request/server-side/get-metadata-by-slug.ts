...
const getMetadataBySlug = async (slug: string): Promise<any> => {
  let res;
  try {
    res = await fetch(`${process.env.NEXT_REQUEST_CMS_STATIC_ROUTE_METADATA_URL!}?where[slug][equals]=${slug}`, {
...
export default getMetadataBySlug;