...
import image1 from "./amplication-1.webp";
...
import image2 from "./amplication-2.webp";
...
- An _action_ when a request hits an API.
- A _query_ in a GraphQL resolver.
- _Business logic_ in service and responses.
...
- Hover your mouse over that channel, then go to the _Edit channel dashboard._

import image3 from "./amplication-3.webp";

<BlogPicture imageMetadata={image3} alt="Edit channel in Discord." />

- Navigate to _Integrations_, then choose _Create Webhooks._

import image4 from "./amplication-4.webp";

<BlogPicture
  imageMetadata={image4}
  alt="Integrate services with Discord via webhooks."
/>

- Your first webhook is automatically created, click on that webhook and _Copy Webhook URL_. Remember, Discord webhooks do not require authentication, so it is sensitive. Do not share the URL publicly.

import image5 from "./amplication-5.webp";

<BlogPicture
  imageMetadata={image5}
  alt="Discord webhook is sensitive, so do not share it publicly."
/>
...
- Navigate to the _Integration Store_ and activate Discord in the _Chat_ section.

import image6 from "./amplication-6.webp";

<BlogPicture
  imageMetadata={image6}
  alt="Novu supports integration with Discord."
/>

- Navigate to the _Notifications_ tab, and create a new template.
- In the _Edit Template_ dashboard, go to the _Workflow_ editor.
- After _Trigger_, add the _Chat_ step. In the _Edit Chat Template_, add the following:`A new product has just arrived, check it out at` [`store.example.com`](https://hungvu.tech/)`!`

import image7 from "./amplication-7.webp";
...
- Upon user creation, create a Novu _subscriber_ using your user id and contact information. This subscriber acts as a service user for the Discord notification functionality.
- Upon receiving a specific event on your backend, retrieve your targeted user(s) from the database.
- Using the user id, _trigger_ _notification templates_ via Novu unified API. This whole step can be inside a loop.
  ...
  notifyDiscordCommunity = async () => {
    try {
      ...
      await this.novu.subscribers.setCredentials(
        process.env.NOVU_SERVICE_USER_ID!,
...
import image8 from "./amplication-8.webp";

<BlogPicture
  imageMetadata={image8}
  alt="Amplication uses Novu as a microservice to distribute messages to Discord channels."
/>
