...
_This article was my contribution to _[_the Medusa blog._](https://medusajs.com/blog/beginner-guide-to-node-js-ecommerce-platform-understanding-the-medusa-server/)

## Medusa Architecture Overview

import image1 from "./medusa-1.webp";

<BlogPicture
  imageMetadata={image1}
  alt="Three main components of a Medusa project."
/>
...
If you have worked with _NestJS_ or _Angular_ before, the folder structure might seem familiar. Navigate to `my-medusa-store`, and you can see the content below.

import image2 from "./medusa-2.webp";
...
Following the [SOLID principle](https://en.wikipedia.org/wiki/SOLID) where an S represents **single responsibility**, files in `/src/api` are controllers responsible for managing HTTP requests/responses and deciding what portion of the code should be called upon.On the other hand, _private inventory access_ is considered a service. Services in Medusa reside in `/src/services`. They represent one specific entity and functionalities related to it.
...
const privateInventoryAccessService = req.scope.resolve(
  "privateInventoryAccessService",
);
