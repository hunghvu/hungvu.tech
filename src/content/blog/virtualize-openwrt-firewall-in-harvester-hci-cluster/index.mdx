...
Harvester offers PCIe passthrough out-of-the-box, but at the time of writing, _this feature is still experimental_. PCIe passthrough is accessible via **Virtual machine config > PCI Devices** web GUI, and the feature just takes a few clicks to enable (assuming your hardware supports that, e.g., VT-d, IOMMU, etc.) There is a technical difference between PCI and PCIe terminologies, but I will use the term PCIe devices in the following sections.

import image1 from "./openwrt-1.webp";

<BlogPicture
  imageMetadata={image1}
  alt="Harvester supports PCIe passthrough via GUI."
/>
...
A very cryptic error message, but in short, the VM was not bootable. This _usually_ happens whenever I reboot the VM by any means (e.g., via OpenWRT GUI, power off and start in Harvester GUI, etc.) Alongside, there were all sorts of random problems when this error message appeared such as:

- VM became unresponsive to actions (via web GUI) like power off, restart, delete, edit-config, and so on.
- VM was indefinitely stuck at the _Starting_ state.
...
import image2 from "./openwrt-2.webp";
...
import image3 from "./openwrt-3.webp";

<BlogPicture
  imageMetadata={image3}
  alt="Passthrough all PCIe devices in the same IOMMU group."
/>
...
- Devices stuck in _In Progress_ state.
- Devices are shown as _Enabled_, but in fact, are not visible in the VM.
- And I simply forgot to pass through one device.

In these cases, the VM could not boot up, or bootable but stuck in _Starting_ state and became unresponsive. There were no indicators, no logs, nothing. Admittedly, I only used web GUI, so maybe something could be seen if using the terminal. This is not a good user experience in my opinion.

On a side note, although I passed through 5 devices in this IOMMU group as shown in the picture, I must not activate the _Power Management Controller (0000:00:1f.2)._ Activating that device always makes my VM not bootable and throws the following error message.
...
import image4 from "./openwrt-4.webp";

<BlogPicture
  imageMetadata={image4}
  alt="Web GUI, notice the system information?"
/>

import image5 from "./openwrt-5.webp";

<BlogPicture imageMetadata={image5} alt="Waveform bufferbloat test result." />

import image6 from "./openwrt-6.webp";

<BlogPicture
  imageMetadata={image6}
  alt="Cloudflare network test result - Internet speed."
/>

import image7 from "./openwrt-7.webp";

<BlogPicture
  imageMetadata={image7}
  alt="Cloudflare network test result - Measurement under different conditions."
/>

import image8 from "./openwrt-8.webp";

<BlogPicture
  imageMetadata={image8}
  alt="OpenWRT system load during Waveform bufferbloat test."
/>

import image9 from "./openwrt-9.webp";

<BlogPicture
  imageMetadata={image9}
  alt="iperf3 from client to OpenWRT result."
/>

import image10 from "./openwrt-1.webp";
...
import image11 from "./openwrt-1.webp";

<BlogPicture
  imageMetadata={image11}
  alt="No major issue found after 45 days."
/>
